...
namespace RoutingKit
{
	...
	void ContractionHierarchy::core_experiment(
		std::vector<unsigned> rank, BitVector const &must_be_core_node, std::vector<unsigned> tail, std::vector<unsigned> head, std::vector<unsigned> weight, const std::string &export_dir, unsigned int degree_stop_criterion_start,
		unsigned int degree_stop_criterion_interval,
		const std::function<void(std::string)> &log_message, unsigned max_pop_count)
	{
		// build_given_rank
		...
		{
			Graph graph(node_count, tail, head, weight);
			build_ch_save_intervals(graph, tail, head, input_arc_id, ch, ch_extra, rank, must_be_core_node, export_dir, degree_stop_criterion_start,
									degree_stop_criterion_interval, max_pop_count, log_message);
		}
	}
